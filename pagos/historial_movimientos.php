...
if (!empty($_GET['tipo'])) {
    if ($_GET['tipo'] === 'ingreso') {
        $where_clauses[] = "m.tipo IN ('ingreso_mensualidad', 'ingreso_otro')";
    } elseif ($_GET['tipo'] === 'gasto') {
        $where_clauses[] = "m.tipo = 'gasto'";
    }
}

if (!empty($_GET['inicio']) && !empty($_GET['fin'])) {
    $where_clauses[] = "m.fecha BETWEEN ? AND ?";
    $types .= 'ss';
    $params[] = $_GET['inicio'];
    $params[] = $_GET['fin'];
}

$sql = "SELECT m.id, m.tipo, m.descripcion, m.monto, m.fecha, 
        ...
        LEFT JOIN categorias_gastos cg ON m.categoria_gasto_id = cg.id";
...
<head>
    ...
    <title>Historial de Movimientos - Mokuso Manager</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        div.dt-container .row:first-child { display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1.5rem; margin-bottom: 1.5rem; }
        div.dt-container .row:last-child { display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1.5rem; margin-top: 1.5rem; }
        ...
        div.dt-container .page-item .page-link { background-color: transparent; border: 2px solid var(--color-border); color: var(--color-text-muted); margin: 0 0.2rem; border-radius: 10px; transition: color 0.3s, border-color 0.3s, background-color 0.3s; box-shadow: none; }
        ...
        div.dt-container .page-item.disabled .page-link { opacity: 0.5; pointer-events: none; background-color: transparent; border-color: var(--color-border); }

        /* --- INICIO DE LA CORRECCIÓN PARA TABLA BLANCA --- */
        .table {
            --bs-table-bg: transparent;
            --bs-table-color: var(--color-text-light);
            --bs-table-border-color: var(--color-border);
            --bs-table-striped-bg: rgba(255, 255, 255, 0.02);
            --bs-table-striped-color: var(--color-text-light);
        ...
        .monto-ingreso { color: var(--color-success); font-weight: 700; }
        .monto-gasto { color: var(--color-error); font-weight: 700; }
        .badge { font-size: 0.8rem; font-weight: 600; padding: 0.4em 1em; border-radius: 20px; }
        .badge-mensualidad { background-color: rgba(0, 191, 255, 0.15); color: var(--color-secondary); }
        .badge-ingreso { background-color: rgba(0, 191, 166, 0.15); color: var(--color-success); }
        .badge-gasto { background-color: rgba(255, 71, 71, 0.15); color: var(--color-error); }
...
<aside class="sidebar">
    ...
    <ul class="sidebar-nav">
        ...
        <li><a href="/MOKUSO/pagos/registrar_pago.php" class="active"><i class="fas fa-dollar-sign"></i> <span>Pagos</span></a></li>
    ...
    <div class="main-container">
        <div class="page-header">
            ...
            <div class="header-actions">
                <a href="index.php" class="btn-ghost"><i class="fas fa-arrow-left me-1"></i> Volver al Dashboard</a>
                <a href="registrar_pago.php" class="btn-primary-gradient"><i class="fas fa-plus-circle me-1"></i> Registrar Movimiento</a>
            </div>
        </div>
                        ...
                        <?php while($row = $result->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td>
                                    <?php
                                        $tipo = $row['tipo'];
                                        $clase_badge = ''; $texto_badge = '';
                                        if ($tipo == 'ingreso_mensualidad') { $clase_badge = 'badge-mensualidad'; $texto_badge = 'Mensualidad'; } 
                                        elseif ($tipo == 'ingreso_otro') { $clase_badge = 'badge-ingreso'; $texto_badge = 'Ingreso'; } 
                                        elseif ($tipo == 'gasto') { $clase_badge = 'badge-gasto'; $texto_badge = 'Gasto'; }
                                    ?>
                                    <span class="badge <?php echo $clase_badge; ?>"><?php echo $texto_badge; ?></span>
                                </td>
                                <td><?php echo htmlspecialchars($row['descripcion']); ?></td>
                                <td>
                                    <?php
                                        $es_gasto = ($row['tipo'] == 'gasto');
                                        $clase_monto = $es_gasto ? 'monto-gasto' : 'monto-ingreso';
                                        $signo = $es_gasto ? '-' : '+';
                                    ?>
                                    <span class="<?php echo $clase_monto; ?>">
                                        <?php echo $signo; ?> $<?php echo number_format($row['monto'], 2); ?>
                                    </span>
                                </td>
                                <td>
                                    <?php
                                        if ($row['tipo'] == 'ingreso_mensualidad' && !empty($row['alumno_nombre'])) {
                                            echo 'Alumno: ' . htmlspecialchars($row['alumno_nombre'] . ' ' . $row['alumno_apellidos']);
                                        } elseif ($row['tipo'] == 'gasto' && !empty($row['categoria_gasto'])) {
                                            echo 'Categoría: ' . htmlspecialchars($row['categoria_gasto']);
                                        } else {
